# All in One Accessibility®: Amiro CMS Plugin
...
To install  All in One Accessibility, need to upload a plugin files in Amiro code, here are the steps:

Step 1: Goto "_local >> plugins_distr" and past the all_in_one_accessibility.zip, and extract it.
Step 2: After extract, in admin panel you can see menu "Мастер плагинов" (EN: Plugin Master), there you can see all plugin list, if the plugin is not available in this list, click on "Обновить список плагинов" (EN: Refresh plugin list),
Step 3: When plugin is available in list, at the end you can see install button to get started, in installation page you have to set "plugins::all_in_one_accessibility" in  "Установить как" (EN: Set as) dropdown, and then click on install
Step 4: Now the plugin is successfully installed, now in side bar you can see "All in One Accessibility" menu under "Мастер плагинов" (EN: Plugin Master) menu to manage plugin settings.
